...
#define WINDOW_CAPTION	"odyssey"				//ウィンドウに表示するタイトル
...
void Draw(void)
{
	...
	//画面クリア
	pDevice->Clear(0, NULL, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER, D3DCOLOR_RGBA(255, 255, 255, 0), 1.0f, 0);//, 全体をクリアする場合はNULL。rect型の配列の要素数を入れるとその形にくり抜ける, TARGET(色)ZBUFFER(深度)のクリア, 
	//第三引数があれば色の指定, ZBUFFERがあれば0が手前、1が奥(一番うしろでクリア),0でok
	pDevice->BeginScene();						//シーンの始まり
	Debug_Begin();
	Game_Draw();
#if (DEBUG) | (_DEBUG)
	DebugFont_Draw(1010, 30, "%.01lf", g_FPS);	//fps表示
#endif
	Debug_End();
